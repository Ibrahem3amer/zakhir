...
@section('content')
	<div class="page_content">
	    ...
	    <h1 class="heading_title">إضافة ألبوم</h1>
	    @if( null === session('error'))

	    @elseif( empty(session('error')) )
	    	<div role="alert" class="alert alert-success"> <strong>تم إضافة الألبوم!</strong></div>
    	@else
	    ...
	    <div class="form">
	        <form class="form-horizontal" action="{{ URL::to('/') }}/admin/editalbum" method="post">
	            ...
	            <div class="form-group">
	                ...
	                <div class="col-sm-10">
	                    <input type="text" value="" name="album_name" class="form-control" id="input0" placeholder="اسم الألبوم">
	                </div>
	            </div>
	            <div class="form-group">
	                ...
	                <div class="col-sm-10">
	                	@foreach( $cats as $opt )
	                		<input type="checkbox" name="check_boxes" class="check_boxes" value="{{$opt->id}}"> {{$opt->name}}<br>
	                	@endforeach
	                </div>
	            </div>
